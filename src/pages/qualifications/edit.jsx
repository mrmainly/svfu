...
import { Form, Typography, Spin, Input, message, DatePicker, Upload, Button } from 'antd'
...
const QualificationEdit = () => {
    ...
    const { data, isFetching } = useGetQualificationsIdQuery({
        id: params.id,
    })
    React.useEffect(() => {
    ...
    return (
        <div>
            ...
            >
                <Form.Item
                    label={<Text style={{ fontWeight: 600, fontSize: 16 }}>Номер документа</Text>}
                    name="doc_id"
                    style={{ width: 350 }}
                >
                ...
                <Form.Item
                    label={
                        <Text style={{ fontWeight: 600, fontSize: 16 }}>Название квалификации</Text>
                    }
                    name="name"
                    style={{ width: 350 }}
                    labelCol={{ span: 24 }}
                ...
                <Form.Item
                    label={<Text style={{ fontWeight: 600, fontSize: 16 }}>Срок действия:</Text>}
                    style={{ width: 350 }}
                    labelCol={{ span: 24 }}
                >
                    <RangePicker
